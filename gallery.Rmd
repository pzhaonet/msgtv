...
```{r hist-geyser,fig.width=4.8, fig.height=4, fig.cap="(ref:fig-hist-geyser)", fig.scap='(ref:fig-hist-geyser-s)', fig.show = 'hold', out.width='50%'}
...
```{r hist-density,fig.width=3.5, fig.height=2, results='hide', tidy=TRUE, fig.cap='(ref:fig-hist-density)', fig.scap='(ref:fig-hist-density-s)', fig.show = 'hold', out.width='50%'}
...
```{r insects-boxplot, fig.width=4.8, fig.height=1.8, tidy=TRUE, fig.cap="(ref:fig-insects-boxplot)", fig.scap="(ref:fig-insects-boxplot-s)", fig.show = 'hold', out.width='50%'}
par(mar = c(2, 2.5, .2, .1))
boxplot(count ~ spray, data = InsectSprays, col = "lightgray", horizontal = TRUE, pch = 4)

InsectSprays %>% 
  ggplot() +
  geom_boxplot(aes(spray, count)) +
  coord_flip()
...
```{r rnorm-boxplot,fig.width=4.8,fig.height=1.2, tidy=FALSE, fig.cap="(ref:fig-rnorm-boxplot)", fig.scap="(ref:fig-rnorm-boxplot-s)", fig.show = 'hold', out.width='50%'}
...
```{r vadeaths-barplot, fig.width=4.8, fig.height=4.8, fig.cap = "(ref:fig-vadeaths-barplot)", fig.scap="(ref:fig-vadeaths-barplot-s)", fig.show = 'hold', out.width='50%', dev= "png"}
...
```{r scatter-alpha, fig.width=4,fig.height=2, dev='png', dev.args=list(type = "cairo"), fig.cap="(ref:fig-scatter-alpha)", fig.scap="(ref:fig-scatter-alpha)", results='hide', fig.show = 'hold', out.width='80%'}
...
```{r assocplot,fig.width=4.8,fig.height=4, fig.cap="(ref:fig-assocplot)", fig.scap= "(ref:fig-assocplot-s)", fig.show = 'hold', out.width='50%', dev= "png"}
...
```{r cdplot,fig.width=4,fig.height=2.5,results="hide", fig.cap="(ref:fig-cdplot)", fig.scap="(ref:fig-cdplot-s)", fig.show = 'hold', out.width='50%', dev= "png"}
...
```{r contour-grid,fig.width=4.8,fig.height=2.5,echo=FALSE,fig.cap="(ref:fig-contour-grid)",fig.scap="(ref:fig-contour-grid-s)", fig.show = 'hold', out.width='50%'}
...
ggplot() +
  geom_polygon(aes(x, y), fill = NA,  colour="black", linetype = 2, 
               data = data.frame(x = c(1, 2, 3, 4, 3, 2), 
                                 y = c(2, 1, 1, 2, 3, 3))) +
  geom_segment(aes(x = 2, y = 2, xend = 3, yend = 2)) + 
  geom_text(aes(x, y, label = z), 
            data = data.frame(x = rep(1:4, 3), 
                              y = rep(1:3, each = 4), 
                              z = paste("z =", z)),
            color = 'blue') +
  xlim(0, 5) + ylim(0, 4)
...
```{r contour-pop,fig.width=4,fig.height=3,results="hide",fig.cap="(ref:fig-contour-pop)",fig.scap="(ref:fig-contour-pop-s)", fig.show = 'hold', out.width='50%'}
...
```{r coplot,fig.width=8,fig.height=7, fig.cap="(ref:fig-coplot)", fig.scap="(ref:fig-coplot-s)", fig.show = 'hold', dev= "png"}
...
```{r curve,fig.width=4.8,fig.height=5,fig.cap="(ref:fig-curve)",fig.scap="(ref:fig-curve-s)", fig.show = 'hold', out.width='50%'}
...
```{r dotchart,fig.width=4.8,fig.height=2.7,fig.cap="(ref:fig-dotchart)",fig.scap="(ref:fig-dotchart-s)", fig.show = 'hold', out.width='50%'}
par(mar = c(4, 4, 0.2, 0.2))
...
```{r image24,fig.width=4,fig.height=1.2,fig.cap="(ref:fig-mage24)",fig.scap="(ref:fig-mage24-s)", fig.show = 'hold'}
par(mar = rep(0, 4))
...
```{r image,fig.height=6,fig.width=6,fig.cap="(ref:fig-image)",fig.scap="(ref:fig-image-s)", fig.show = 'hold'}
...
```{r matplot,fig.width=4.8,fig.height=3.5,fig.cap="(ref:fig-matplot)",fig.scap="(ref:fig-matplot-s)", fig.show = 'hold', out.width='50%'}
...
```{r mosaicplot,fig=T,fig.width=5.2,fig.height=5,fig.cap="(ref:fig-mosaicplot)",fig.scap="(ref:fig-mosaicplot-s)", fig.show="hold"}
par(mar = c(2, 3.5, .1, .1))
mosaicplot(Titanic, shade = TRUE, main = "")

#' Standardized residuals of a loglinear model 
#' 
#' @param x a contingency table
#' @param margin a list of vectors with the marginal totals to be fit in the log-linear model
#' @param type a character string indicating the type of residual to be represented. 
#' Must be one of "pearson" (giving components of Pearson's chi-squared), "deviance" 
#' (giving components of the likelihood ratio chi-squared), or "FT" for the 
#' Freeman-Tukey residuals. The value of this argument can be abbreviated.
mosaic_residual <- function(x, margin = NULL, type = c("pearson", "deviance", "FT")) {
  if (is.null(margin)) { 
    margin <- as.list(1:length(dim(x)))
  }
...
titannic_tidy <- mosaic_residual(Titanic) %>% 
  as.data.frame() %>% 
  rename(residuals = Freq) %>% 
  left_join(as.data.frame(Titanic), .) %>% 
  mutate(
    Sex = factor(Sex, levels = c("Female", "Male")),
    Survived = factor(Survived, levels = c("Yes", "No"))
  ) %>% 
  arrange(Sex, Survived, Age) %>% # 与原图顺序一致
  mutate(
    standardized_residuals = cut(residuals, c( -Inf, -4, -2, 0, 2, 4, Inf)),
    Freq = ifelse(Freq == 0, Freq + 0.5, Freq), # Freq为0时，显示较小矩形
    class_age = factor(paste(Class, Age, sep = ":"), 
      levels = paste(rep(levels(Class), each = 2), levels(Age), sep = ":"),
    ),
    sex_survived = paste(Sex, Survived, sep = ":")
  ) %>% 
  group_by(class_age) %>% 
  mutate(group_count = sum(Freq))

labels_breaks <- filter(titannic_tidy, class_age == "1st:Adult") %>% 
  mutate(breaks = (cumsum(Freq) - 0.5 * Freq)/group_count)

ggplot(titannic_tidy, aes(class_age, Freq, group = class_age)) +
  geom_bar(aes(fill = standardized_residuals, width = group_count),
    stat = "identity", position = "fill", colour = "white") +
  facet_grid(~class_age, scales = "free_x", space = "free_x") +
  coord_cartesian(clip = "off") +
  scale_y_continuous(breaks = labels_breaks$breaks, 
    labels = labels_breaks$sex_survived
  ) +
  labs(y = "Sex:Survived") +
  guides(fill = guide_legend(title = "Standardized\nResiduals", 
...
```{r pairs,fig.width=6,fig.height=6,fig.cap="(ref:fig-pairs)",fig.scap="(ref:fig-pairs-s)", fig.show = 'hold'}
...
GGally::ggpairs(iris, aes(color = Species, alpha = 0.2), 
...
```{r rug,fig.width=4.8,fig.height=2,fig.cap="(ref:fig-rug)",fig.scap="(ref:fig-rug-s)", fig.show = 'hold', out.width='50%'}
...
```{r smoothScatter,fig.width=5,fig.height=5,tidy =FALSE,fig.cap="(ref:fig-smoothScatter)",fig.scap="(ref:fig-smoothScatter-s)",dev='png', dev.args=list(type = "cairo")}
...
```{r sunflowerplot,fig.width=4,fig.height=4,fig.cap="(ref:fig-sunflowerplot)",fig.scap="(ref:fig-sunflowerplot-s)",fig.show="hold"}
par(mar = c(4, 4, 0.2, 0.2))
sunflowerplot(iris[, 3:4], col = "gold", seg.col = "gold")

#' Get coordinates of flower leaves
#' 
#' @param x,y numberic vector of length one
#' @param number number of the overplot points `(x, y)`
#' @param size of sunflower leaves in inches, 1[in] := 2.54[cm]. Default: 1/8, 
#' approximately 3.2mm.
#' @param p ggplot object, points
get_leaves <- function(x, y, number, p, size = 1/8) {
  degree <- purrr::map_dbl(seq_len(number), ~ (2 * pi * .x)/number)
  
  degree <- (2 * pi)/number * seq_len(number)
  
  # sunflower leaves in inches, code was adapted from the source of function
  # graphics:::sunflowerplot.default()
  ppin <- par("pin")
  p_scales <- layer_scales(p)
  xrange <- p_scales$x$range$range
  yrange <- p_scales$y$range$range
  xr <- size * abs(xrange[2] - xrange[1])/ppin[1]
  yr <- size * abs(yrange[2] - yrange[1])/ppin[2]
...
sunflower <- xyTable(iris[, 3:4]) %>% 
  bind_cols()
is_multiple <- sunflower$number > 1
p <- ggplot() +
  geom_point(aes(x, y), sunflower, col = "gold")
leaves <- purrr::map_df(sunflower, ~ .x[which(is_multiple)]) %>% 
  purrr::pmap(get_leaves, p = p) %>% 
  bind_rows()
...
```{r vioplot,fig.width=4.8,fig.height=4,fig.cap="(ref:fig-vioplot)",fig.scap="(ref:fig-vioplot-s)", fig.show = 'hold', out.width='50%'}
f <- function(mu1, mu2)
...
```{r faces2,fig.width=4.8,fig.height=5.2,fig.cap="(ref:fig-faces2)",fig.scap="(ref:fig-faces2-s)"}
...
```{r andrews-curve,fig.width=4.8,fig.height=4.8,fig.cap="(ref:fig-andrews-curve)",fig.scap="(ref:fig-andrews-curve-s)"}
...
```{r bagplot,fig.width=4.8,fig.height=3,fig.cap="(ref:fig-bagplot)",fig.scap="(ref:fig-bagplot-s)"}
...
```{r windrose,fig.width=5.4,fig.height=4.6,sanitize=TRUE,results="hide",fig.cap="(ref:fig-windrose)",fig.scap="(ref:fig-windrose-s)"}
...
7.  在信息可视化（Information
    ...
    knitr::include_graphics("images/disk-usage.png")
    ```
    ...
    knitr::include_graphics("images/yihui-name-wordle.png")
    ```
